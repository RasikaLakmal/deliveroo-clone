...
export const basketSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromBasket2: (state, action) => {
      const realid = state.items.map((obj) => obj.id )
        const index = realid.findIndex((item) => item );
        let newBasket = [...state.items];
...
export const { addToBasket, removeFromBasket, removeFromBasket2 } = basketSlice.actions
